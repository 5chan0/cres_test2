...
def main():
    ...
    criterion = nn.MSELoss(reduction='sum').cuda()  # size_average=False -> reduction='sum'
...
def validate(val_list, model, criterion, args):
    ...
    with torch.no_grad():
        for i, (img, target) in enumerate(test_loader):
            ...
            # target.sum()은 CPU 텐서. output.data.sum()은 GPU 텐서이므로, 둘 다 GPU 텐서로 변환 후 계산.
...
def adjust_learning_rate(optimizer, epoch, args):
    """Sets the learning rate to the initial LR decayed by the args.scales schedule."""
