...
class CSRNet(nn.Module):
    def __init__(self, load_weights=False):
        ...
        # ------------------------------------------------------------------------
        # 원래 ResNet50 해상도 변환 흐름: /4 (stem) -> /8 (layer2) -> /16 (layer3) -> /32 (layer4)
        # 목표: 최종 /8로 만들기 위해 layer3, layer4 모두 stride=1 + dilation 적용 필요.
        # ------------------------------------------------------------------------
        
        # (1) layer3: 기본 stride=2 -> stride=1로 바꾸고, dilation=2로 설정 → 출력 해상도 /8 유지
        ...
        # (2) layer4: 기존 stride=2 -> stride=1, dilation=4로 설정 → 최종 여전히 /8 (receptive field가 더 커짐)
        ...
        self.frontend = nn.Sequential(
            ...
            backbone.layer1,  # /4
            backbone.layer2,  # /8
            backbone.layer3,  # /8 (dilated, stride=1)
            backbone.layer4   # /8 (dilated, stride=1)
        )
        
        # ========== Backend는 기존 CSRNet 구조 그대로 ==========
        # ResNet50 최종 출력 채널: 2048
    ...
    def forward(self, x):
        x = self.frontend(x)      # Dilated ResNet50 → 최종 /8 해상도
